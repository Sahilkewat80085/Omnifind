 ...
 *
 * The backend now collapses per file before it trims to top-k — it has to,
 * or a long file's chunks eat every slot and the page is handed a single
 * result. This is the safety net for anything that reaches the UI with
 * duplicates anyway; results arrive sorted, so the first appearance of a file
 * is already its strongest hit.
 */
